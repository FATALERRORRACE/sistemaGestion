...
class InscritosController extends Controller
{
    ...
    public function get(Request $request){
        ...
        $query = UsuariosDb::
        scopeDocumento($request->documento)
        ->scopeNombres($request->nombres)
        ->scopeApellido($request->apellidos)
        ->scopeEmail($request->email)
        ->scopeConsecutivo($request->consecutivo);

        
        DB::table('usuarios_bd')->where;
        //from institucion where Documento = '".$_POST["n_documento"]."' 
	    //$query = "select Consecutivo, Biblioteca, Fecha_Solicitud, Fecha_Activacion, concat(Nombres, ' ', Apellidos) as Nombre, N_Documento, Estado, T_Afiliado 
